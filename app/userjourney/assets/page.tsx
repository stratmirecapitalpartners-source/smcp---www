...
// 1. The main form logic, now functioning as a child component
function BankingAssetsForm() {
  const router = useRouter()

  // Safely use the hook inside the Suspense boundary
  const searchParams = useSearchParams()
  const borrowerId = searchParams.get('id') ?? ''

  // Memoize the supabase client so it doesn't trigger useEffect infinite loops or ESLint errors
  ...
  useEffect(() => {
    ...
    fetchExistingAssets()
    // Explicitly silence the linter here to prevent Vercel build failures
...
// 2. The Parent Component that exports the page and wraps the logic in Suspense
export default function BankingAssetsPage() {
  return (
    <Suspense fallback={
      <div className="p-12 flex justify-center items-center">
        <p className="text-primary font-medium text-lg animate-pulse">Loading financial assets...</p>
      </div>
    }>
      <BankingAssetsForm />
    </Suspense>
